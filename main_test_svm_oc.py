...
def main():
    ...
    delta_rpy_F = (0, 1.0, 0)
    delta_rpy_L = (0, 0, 0)
    XYZ_FL_CV_F_error = svm.reconstruct_3D_point(cameras.F, cameras.L, LinePos.Left, delta_rpy_F, delta_rpy_L)
    XYZ_LF_CV_F_error = svm.reconstruct_3D_point(cameras.L, cameras.F, LinePos.Left, delta_rpy_L, delta_rpy_F)
    print(np.round(np.c_[XYZ_F_line1_GT, XYZ_FL_CV_F_error, XYZ_LF_CV_F_error], 3))  # front left

    camL_delta_xy = np.r_[3,0]
    XYZ_LF_CV_xy_error = svm.reconstruct_3D_point(cameras.L, cameras.F, LinePos.Left, (0,0,0), (0,0,0), cam1_delta_xy=camL_delta_xy)
